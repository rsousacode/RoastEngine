...
namespace ImGuiMetal {
    bool Init(id<MTLDevice> device)
    {
        ImGuiIO& io = ImGui::GetIO();
        io.BackendRendererName = "imgui_impl_metal2";
    ...
    {
        IM_ASSERT(g_sharedMetalContext != nil && "No Metal context. Did you call Init() ?");
...
// Metal Render function.
    void RenderDrawData(ImDrawData* draw_data, id<MTLCommandBuffer> commandBuffer, id<MTLRenderCommandEncoder> commandEncoder)
    {
        [g_sharedMetalContext renderDrawData:draw_data commandBuffer:commandBuffer commandEncoder:commandEncoder];
    }

    bool CreateFontsTexture(id<MTLDevice> device)
    {
    ...
    bool CreateDeviceObjects(id<MTLDevice> device)
    {
